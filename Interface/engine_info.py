...
class EngineImageAndInfo(ctk.CTkFrame):     # Class for displaying basic engine info
    def __init__(self,master):
        ...
        # Label
        self.image_info_label = ctk.CTkLabel(self.image_info_frame, text="SELECT ENGINE RANGE AND MODEL")
    ...
    def set_image(self, engine_range, engine_series):
        ...
        if engine_range == "MTU":
            ...
            self.range_label.configure(text="MTU RANGE")
        if engine_range == "Perkins":
            self.image_info_label.configure(text="", image=self.perkins_image)
            self.range_label.configure(text="PERKINS RANGE")
        if engine_range == "Scania":
            self.image_info_label.configure(text="", image=self.scania_image)
            self.range_label.configure(text="SCANIA RANGE")
            
        self.series_label.configure(text=f"SERIES: {engine_series}")
